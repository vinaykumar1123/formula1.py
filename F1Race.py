...
pygame.display.set_caption("Vinay Kumar Ki Race Game")
    ...
    while pause:
        for event in pygame.event.get():
            if event.type == pygame.QUIT or (
                    event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE):  ###############or quit_game == 1
                pygame.quit()
                quit()
...
def looping_gameplay():
    ...
    while not gameExit:
        ...
        if height_y < th_st_y + t_height - 15 and th_st_x - c_width - 5 < width_x < th_st_x + t_width - 5:
            crash_function()
